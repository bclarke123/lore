...
| `auto` | `BackendKind::Auto` | Selects the platform's completion backend where it can be created, and `psync` otherwise. |

The probe never fails. A machine that cannot provide a ring — an older kernel, a `seccomp` policy, a container — receives `psync`, because `auto` requests the best available backend rather than a specific one. `BackendKind::Uring` and `BackendKind::Iocp` are how a caller states that the completion backend is required and that a failure to create one must surface.

`auto` prefers a completion backend on both platforms that have one, and neither preference is a settled throughput win:

- On Linux the ring measures 1.60× the syscall pool for commit-shaped reads on ext4 and 0.70× to 0.90× for the other repository-shaped cases, so the preference favours read-heavy work on a filesystem that completes cached reads during the submit syscall.
- On Windows the decision rests on threads rather than throughput. A positional operation defers on that platform, so `psync` parks a pool thread for the duration of every operation and its cap bounds how many can be in flight. `iocp` reaches 1.76× and 4.80× on the synthetic read phases while running two threads against a saturated thirty-two. It measures 0.72× on the commit-shaped read phase.

Both preferences are provisional while the migration is internal and nothing depends on the engine. The eventual selection criterion on Linux is behavioural rather than nominal: submit one read, observe whether it completed during the submit syscall, and choose per machine. Keying on the observation rather than on a filesystem name also classifies tmpfs correctly without enumerating filesystems.
