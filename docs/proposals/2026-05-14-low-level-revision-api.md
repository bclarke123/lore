...
created: 2026-05-14
updated: 2026-08-17
discussion: https://crowd.urc.internal.epicgames.net/epic/Lore/change-request/360
---
...
**Storage handle lifecycle.** A revision handle holds its own Arc reference to the underlying `StoreInternal`, the same way two storage handle opens against the same path share an `Arc<Store>`. Closing the storage handle marks that storage handle entry invalid but does not tear the store down until every revision handle on it also closes. New API calls against a closed storage handle's revision handles still succeed against the underlying store — `lore_storage_close` does not invalidate the revision handle's view. **Resolved: no warning.** The close reports nothing and closes nothing — the revision handles stay fully usable, so there is no lost work to surface and nothing for the caller to act on. A warning is only read by a human when it reaches the CLI, and the CLI drives neither this API nor the storage API. Releasing the revision handles stays the caller's job, with `lore_revision_tree_close`. Two paths do close them, and neither is application-driven: when an IPC connection drops, the dispatcher's teardown closes the revision handles its storage handles owned, because nobody is left to; and `lore_shutdown` closes every outstanding revision handle, before the storage handles they were loaded against.
...
Appended during implementation. The body above is the proposal as accepted and is left as accepted; where the built surface diverges, this section is authoritative. The design spec and its errata carry the reasoning.

- **Every write verb is a batch.** `add`, `delete`, `modify`, `move`, `metadata_set`, `metadata_get` and `metadata_clear` take `verb(handle, entries[])` rather than the single-item signatures sketched under "Write operations". A one-entry call is the one-entry case; there is no separate single-item entry point. It collapses N dispatches into one, removes the quadratic cost of N single adds into one directory, and is the transaction boundary that closes the check-then-act race between a verb's validation scan and its mutation. Mutating batches are all-or-nothing: every entry is validated before anything is applied.

- **`move` is the one verb whose batch fans out nowhere, and whose batch rules read the whole call.** A move unlinks a node from one child chain and links it into another, where every other write verb either only prepends or touches no chain at all, so entries apply one at a time in batch order rather than being analysed for independence. Both of its batch-level rules need more than a per-entry check: a loop is settled against the reparenting the earlier entries in the call perform, since moving A under B and B under A is legal twice over and a loop once; and a destination name is settled once every entry's destination is known, so a name the batch itself vacates is free — moving `x` out of a directory while moving another node to `x` in it succeeds, and two entries taking one name reject. The name check is the verb's rather than the state primitive's for exactly this reason: held against the tree in front of it, no ordering of a legal swap would pass. A moved directory records its subtree as moved with it, matching what the working-tree move paths already write into a revision — `lore file history` reads a per-node record, so a file that moved with its parent would otherwise read back as deleted at that revision. That per-node record is not the intended end state, and is written this way only to match what the file-based paths write today: a move should mark the node it moves and nothing else, with a directory's children implicit in the parent's record — the subtree travels by its parent pointers and nothing about a descendant changes. Narrowing it is a change against the file-based surface rather than this API, because the readers of the record — file history, and the streaming diff — have to learn to look for a moved ancestor first, and revisions already published carry the per-node shape.
...
- **Tip-collision behaviour is resolved**, closing the first unresolved question: `commit` fails and the caller decides. No automatic fast-forward, no rebase, no write-only mode. The failure carries the observed tip so the caller can reload without a second query. Concurrent commits are *not* serialized by the handle's in-flight machinery as "Non-Functional Considerations" assumes — commits on separate handles or processes race through the pipeline, and a real compare-and-swap on the branch tip is what makes at most one publish.

- **Every call claims the handle's tree before touching it, and says whether it can share it.** Every verb takes a *shared* claim: `State` is internally mutable, so an edit applies under one, and what a shared claim excludes is a commit rather than another edit. `commit` takes the *exclusive* claim, for the whole call. This is not a convention — the state is private and reachable only through the two claim types, so a call cannot get at the tree without stating which one it takes.

  `metadata_set` and `metadata_clear` claim the handle too, though they touch no tree: the pending metadata is what a commit clones and then empties, so an edit landing inside a commit would be recorded on the handle and then dropped without ever reaching a revision. Claiming it is what removes the "do not call `metadata_set` during a commit" obligation a caller would otherwise carry, alongside the one about two in-flight commits on a handle agreeing on `remote_write`.

  A caller whose event callback re-enters the API on the same handle deadlocks against the claim rather than corrupting a commit — which the callback contract already forbids.

- **A commit either publishes or leaves the handle as it found it.** It snapshots the staged tree just before the freeze, under the exclusive claim. Either it succeeds and the handle is left consistent on the new revision, or it fails, the part-frozen tree is discarded and rebuilt from that snapshot, and the handle sits on the revision it was loaded at with its edits still staged and still committable. Recovery is to fix what the terminal reported and retry on the same handle — no close, no reload, no re-applying edits. The one failure that still invalidates a handle is a restore that itself fails, leaving a tree neither committed nor restored.

  Concurrent commits on one handle serialize on the exclusive claim. This does **not** change the `(parent, name)` note below: concurrent `add` calls hold shared claims and can still race a name, with the commit-time validator still the net.

  Two costs are accepted rather than solved. A commit blocks every other call on its handle for its duration — correct rather than fast, since those calls would otherwise observe a half-frozen tree. And the snapshot roughly doubles a commit's write volume, leaving blocks that nothing references once it succeeds; they are content-addressed and collectable, the same category as the orphan blocks a losing committer already leaves.
...
Decisions settled during the review this proposal was accepted on, recorded because the reasoning is not recoverable from the surface it produced. Reasoning rather than contract — the sections above are what binds.

### A failed commit restores the handle instead of poisoning it

The freeze rewrites the handle's tree in place, so a commit that fails partway leaves a part-frozen tree behind. The first answer to that was to poison the handle: mark it unusable, and require the caller to close it, load a fresh handle from the parent revision, and re-apply every edit the failed commit had staged. For the audience this LEP is written for — importers and mirrors staging thousands of edits per revision — that turns any transient commit failure into a full replay.

Narrowing the poison to "only once the freeze has begun" was tried and rejected as the wrong fix, because it left the poison in place for the cases most likely to be hit while making the contract harder to state, not easier. What settled it was one condition the narrowing answered two ways. A lost branch-tip race resolves differently depending only on whether the winner published before or after the freeze began: lose it early and the terminal names the tip to reload from, on a handle still able to do the reloading; lose it late and it names the same tip *and* poisons the handle that would have used it. One condition, two outcomes, selected by timing the caller can neither observe nor control — not a contract that can be written down, which is the sign that the poison itself was the defect rather than its breadth.

The claim model in the Implementation Notes above is what replaced it. A commit takes the handle exclusively, snapshots the staged tree just before the freeze, and on any failure discards the part-frozen tree and rebuilds from that snapshot. Discarding rather than repairing is deliberate: a part-frozen tree has no invariant to repair *toward*, whereas the snapshot is a state the caller already had. The exclusive claim is what makes the swap sound — nothing can be reading the tree being replaced — and it is also what closes the window where an edit or a metadata change lands inside a commit. Poisoning survives only where the restore itself fails, which is genuinely unrecoverable rather than a choice.

The costs were weighed and accepted: a commit blocks every other call on its handle for its duration, and the snapshot roughly doubles a commit's write volume. Both are paid per commit, which is the coarsest operation on the surface and already the one a pipeline batches into. Two caller obligations disappear in exchange — "do not call `metadata_set` during a commit" and "two in-flight commits on one handle must agree on `remote_write`" — neither of which was a rule a caller could reasonably be asked to keep, and both of which existed only because commit had no exclusive claim.
