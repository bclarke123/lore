...
/// `RepositoryAuthorizer` seam, which every enforcement point (gRPC
/// partition layer, QUIC session start, HTTP middleware, cross-partition
/// link reads) now consults. Anonymous callers — no token, or the
/// [`crate::auth::anonymous`] sentinel — get read-level visibility on
/// public repositories, the same rule the anonymous tower layer applies at
/// the interceptor seam. Any role reaches the repository; a privileged
...
impl GrantStoreRepositoryAuthorizer {
    ...
    async fn public(&self, repository_id: RepositoryId) -> Result<(), Status> {
        match self.access.is_public(repository_id).await {
            Ok(true) => Ok(()),
...
impl RepositoryAuthorizer for GrantStoreRepositoryAuthorizer {
    async fn check_repository_access(
        ...
        let claims = match token {
            None => return self.public(repository_id).await,
            Some(token) if crate::auth::anonymous::is_anonymous(token.claims) => {
                return self.public(repository_id).await;
            }
            Some(token) => token.claims,
        };
        let principals = crate::access::Principals::from_claims(claims);
        match self.access.role_for(&principals, repository_id).await {
            Ok(Some(crate::access::AccessRole::Admin)) => Ok(()),
            Ok(Some(_)) if action.is_none() => Ok(()),
            Ok(Some(_)) => Err(Status::permission_denied(format!(
                "{} requires the admin role on this repository",
    ...
    // `granted_actions` stays at the trait default (`None`): the grant
    // store answers per question, so handlers fall back to
    // `check_repository_access` for privileged actions.
    //
    // `check_repository_access_sync` also stays at the default: grants
    // live in the store, not the token, so cross-partition link reads are
    // denied — as they were before, when any token without a `resources`
    // claim was refused a link read.
}
...
pub fn repository_authorizer(
    ...
    // The server-local grant store, when installed, outranks every other
    // tier: the decision is the same one the external auth service would
    // answer from, minus the round trip.
    if let Some(access) = crate::access::installed() {
        info!("Repository authorizer: GrantStoreRepositoryAuthorizer");
        return Ok(Arc::new(GrantStoreRepositoryAuthorizer::new(access)));
    }
...
mod grant_store_tests {
    ...
    async fn grant_store_authorizer_answers_from_grants() {
        ...
        Box::pin(LORE_CONTEXT.scope(execution, async move {
            ...
            let anonymous_claims = crate::auth::anonymous::anonymous_token(project);
            let sentinel = VerifiedToken {
                raw: "",
            ...
            let denied = authorizer
                .check_repository_access(Some(&alice), project, None)
                .await
                .expect_err("no grant");
            assert_eq!(denied.code(), Code::PermissionDenied);
            let denied = authorizer
                .check_repository_access(None, project, None)
                .await
                .expect_err("anonymous on private");
            assert_eq!(denied.code(), Code::PermissionDenied);

            // A grant admits the bearer, but a privileged action needs admin.
            access
            ...
            authorizer
                ...
                .expect("granted");
            let denied = authorizer
                .check_repository_access(Some(&alice), project, Some("obliterate"))
                .await
                .expect_err("reader may not obliterate");
            assert_eq!(denied.code(), Code::PermissionDenied);
            access
            ...
            authorizer
                .check_repository_access(Some(&alice), project, Some("obliterate"))
                .await
                .expect("admin may obliterate");

            // The public grant admits anonymous callers, plain and via the
            // admitted-anonymous sentinel; the sentinel never gets actions.
            access
            ...
            authorizer
                .check_repository_access(Some(&sentinel), project, None)
                .await
                .expect("sentinel on public");

            // Grants live in the store, so the sync path cannot answer.
            assert!(
                authorizer
                    .check_repository_access_sync(Some(&alice), project, None)
                    .is_none()
            );
        }))
        .await;
